...
def get_devices_by_class(devclass):
    ...
    """
    # usar replace em vez de format() porque o script PowerShell contém muitas chaves
    # que causariam conflitos com str.format(). Substitui apenas o placeholder {cls}.
    out = run_powershell(ps.replace('{cls}', devclass))
    items = []
    try:
        parsed = json.loads(out) if out else []
